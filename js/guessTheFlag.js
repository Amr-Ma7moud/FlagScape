...
function setupTypingMode() {
    const allCountryNames = state.countries.map(c => c.name.common).sort();
    let selectedIndex = -1;

    const handleInput = () => {
        const input = els.typeInput.value.trim();
        if (!input) {
            els.autocomplete.innerHTML = "";
            return;
        }
        ...
        matches.forEach((name, idx) => {
            ...
            div.onclick = () => submitTypedAnswer(name);
    ...
    const handleKeydown = (e) => {
        ...
        if (e.key === "ArrowDown") {
            ...
            if (selectedIndex >= 0 && items[selectedIndex]) {
                submitTypedAnswer(items[selectedIndex].textContent);
            } else {
                submitTypedAnswer(els.typeInput.value);
            }
        }
    };
    ...
    const submitTypedAnswer = (answer) => {
    ...
    els.typeInput.addEventListener("input", handleInput);
    els.typeInput.addEventListener("keydown", handleKeydown);
}
