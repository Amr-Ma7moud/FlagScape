      ...
      countryPaths.forEach(country => {
        ...
        country.addEventListener('click', (e) => {
          ...
          if (name !== 'Unknown') {
            ...
            document.querySelectorAll('.country-clicked').forEach(el => {
              el.classList.remove('country-clicked');
            });
            mapContainer.classList.remove('zooming');
            
            // Add zoom animation to container and highlight country
            country.classList.add('country-clicked');
            mapContainer.classList.add('zooming');
            console.log('Zoom animation triggered for:', name);
            
            // Open modal after zoom completes
            setTimeout(() => {
              countryModal.open(name);
              // Reset zoom after modal opens
              setTimeout(() => {
                country.classList.remove('country-clicked');
                mapContainer.classList.remove('zooming');
              }, 300);
            }, 1000);
            }, 200);
            ...
            country.addEventListener('animationend', () => {
              country.classList.remove('country-clicked');
            }, { once: true });
          }
        });
